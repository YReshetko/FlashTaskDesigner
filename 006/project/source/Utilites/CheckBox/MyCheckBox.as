package source.Utilites.CheckBox {
	...
	public class MyCheckBox extends Sprite{
		...
		private function THIS_SAMPLE_SELECT(e:Event){
			setCurrentData(e.target.getLabel(), e.target.getData());
		...
		public function setCurrentData(label:String, data:*):void{
			changeData(label);
			currentData = data;
			THIS_DEACTIVE(null);
		}
		public function setData(value:*):void{
			...
			l = arrSample.length;
			for(i=0;i<l;i++){
				if((arrSample[i] as SampleCheck).getData() == value){
					setCurrentData((arrSample[i] as SampleCheck).getLabel(), (arrSample[i] as SampleCheck).getData());
					return;
				}
			}
		}
